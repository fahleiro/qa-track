...
echo "--- Iniciando Postgres (porta $POSTGRES_PORT) ---"
...
su - postgres -c "psql -p $POSTGRES_PORT -c \"CREATE DATABASE qa_test_track;\"" 2>/dev/null || echo "Database já existe"

echo "=== Postgres configurado com sucesso (porta $POSTGRES_PORT) ==="
