...
async function testCreateStatus(): Promise<void> {
  ...
  await step('03_preencher_e_adicionar', async () => {
    await fillInput(driver!, By.css('input[placeholder="Nome do status"]'), TEST_NAME)
...
async function testValidateDb(): Promise<void> {
  ...
  await step('04_validar_no_db', async () => {
    ...
    if (!result.exists) throw new Error(`Status "${TEST_NAME}" não encontrado no DB`)
    log.info(`ID no DB: ${result.data?.id}`)
  })
}

async function testEditStatus(): Promise<void> {
  log.section('WEB — Editar status')
...
async function testDeleteStatus(): Promise<void> {
  log.section('WEB — Remover status')
...
async function cleanup(): Promise<void> {
  ...
  await deleteStatusByTitle(TEST_NAME_EDITED).catch(() => deleteStatusByTitle(TEST_NAME))
...
export async function run(): Promise<void> {
  ...
  try {
    ...
    await testEditStatus()
    await testDeleteStatus()
  } finally {
    await cleanup()
  }
}
